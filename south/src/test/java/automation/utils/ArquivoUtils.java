...
public class ArquivoUtils {
	...
	public static void deleteFilesArquivos(){
		String out = new File(TestConfig.resourceArquivoPath()).toString();
		File file = new File(out);
		File afile[] = file.listFiles();
		for (File f : afile) {
			f.delete();
...
//		
	...
	public static InputStream projectResourceAsStream(String name) {
		InputStream url =Thread.currentThread().getContextClassLoader().getResourceAsStream(name);
